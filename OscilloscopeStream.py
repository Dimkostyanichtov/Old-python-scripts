...
if scp:
    try:
        ...
        scp.sample_frequency = 1e7  # 1 kHz - частота сэмплирования
        
        # Set record length:
        scp.record_length = 10000  # 1 kS - длина блока
        
        scp.resolution = 16 # разрешение АЦП
        ...
        for ch in scp.channels:
            ...
            # Set range:
            ch.range = 8  # 8 V - диапазон значений АЦП

            # Set coupling:
            ch.coupling = libtiepie.CK_DCV  # DC Volt - тип измеряемого сигнала
            ch.safe_ground_enabled = True # включение SafeGround
            ch.safe_ground_threshold = 0.1 # порог SafeGround
        ...
        try:
            ...
            print()
            sample = 0
            for chunk in range(1): # Количество блоков данных
                print(chunk)
